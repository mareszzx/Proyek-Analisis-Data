...
# --- 1. OPTIMASI PERFORMA DENGAN CACHING ---
@st.cache_data
def load_data():
...
with st.sidebar:
    ...
    # Filter 2: Customer State (Multi-select)
    st.markdown("---")
    state_options = sorted(all_df["customer_state"].unique())
...
if main_df.empty:
    st.warning("⚠️ Data tidak tersedia untuk rentang waktu atau filter yang dipilih. Silakan ubah filter Anda.")
    st.stop()
...
with col1:
    ...
    st.metric("Overall Total Revenue", value=total_rev)
...
ax.set_title("Distribution of Customer Segments", fontsize=15)
st.pyplot(fig)
